...
fn add_moves(pos: &Position, from: Square, mut bb: Bitboard, moves: &mut Vec<ScoredMove>) {
    while bb != 0 {
        ...
        moves.push(ScoredMove::new(Move::new(
            from,
            to,
            pos.piece_at(from),
            pos.piece_at(to),
...
fn add_castles(pos: &Position, moves: &mut Vec<ScoredMove>) {
    ...
    for side in 0..2 {
        ...
        if !failed && ci.may_discover_check {
            ...
            // castling rook.
            if bitboard::rook_attacks(ci.kdest, pos.all_pieces() ^ bb!(ci.rook))
                & (pos.pieces_of_color_and_type(them, PieceType::Rook)
                    | pos.pieces_of_color_and_type(them, PieceType::Queen))
                != 0
            {
                failed = true;
            }
        ...
        if !failed {
            moves.push(ScoredMove::new(Move::new_castle(
                ci.king,
                ci.rook,
                pos.piece_at(ci.king),
            )));
        }
    }
}

fn add_pawn_promotions(
    pos: &Position,
    pawns: Bitboard,
    mut mask: Bitboard,
    d: Delta,
    ...
    while mask != 0 {
        ...
        if gt != GenerationType::Quiet {
            moves.push(ScoredMove::new(Move::new_promotion(
                from,
                to,
                pos.piece_at(from),
                pos.piece_at(to),
...
fn add_masked_pawn_moves(
    ...
    let push_twice_mask = bitboard::shift(bitboard::shift(pawns, up) & empty, up)
        & empty
        & bitboard::relative_rank_bb(us, Rank::_4)
        & mask;
    ...
    if gt != GenerationType::Quiet && pos.ep_square() != Square::NoSquare {
        ...
        while bb != 0 {
            moves.push(ScoredMove::new(Move::new_en_passant(
                bitboard::pop_square(&mut bb),
                pos.ep_square(),
...
const LEGAL_PHASES: &'static [SelectionPhase] = &[
    SelectionPhase::Start,
    SelectionPhase::Legal,
    SelectionPhase::Done,
];
const ROOT_PHASES: &'static [SelectionPhase] = &[
    SelectionPhase::Start,
    SelectionPhase::Root,
    SelectionPhase::Done,
];
const NORMAL_PHASES: &'static [SelectionPhase] = &[
    SelectionPhase::Start,
    SelectionPhase::TT,
    SelectionPhase::Loud,
...
const EVASION_PHASES: &'static [SelectionPhase] = &[
    SelectionPhase::Start,
    SelectionPhase::TT,
    SelectionPhase::Evasions,
    SelectionPhase::Done,
];
const QUIESCENCE_PHASES: &'static [SelectionPhase] = &[
    SelectionPhase::Start,
    SelectionPhase::TT,
        ...
        match self.phases[self.phase_index] {
            ...
            SelectionPhase::Quiet => {
                for m in self.moves.iter_mut() {
                    ...
                    if m.m == self.killers[0] || m.m == self.killers[1] || m.m == self.countermove {
    ...
    pub fn next(
        ...
        loop {
            ...
            if phase == SelectionPhase::Loud {
                ...
                if sm.m == self.killers[0] || sm.m == self.killers[1] || sm.m == self.countermove {
                    continue;
                }
...
mod tests {
    ...
    chess_test!(test_move_is_pseudo_legal, {
        ...
        test_case(
            "8/2k5/8/8/8/8/2q2K2/8 w - -",
            Move::new(E2, D1, WK, NoPiece),
            false,
        );
        test_case(
            "8/2k5/8/8/8/8/2q2K2/8 w - -",
            Move::new(E2, F2, WK, NoPiece),
            false,
        );
        test_case(
            "6q1/2k5/8/8/8/8/8/4K2R w K -",
            Move::new_castle(E1, H1, WK),
            false,
        );
        test_case(
            "8/2k5/8/5P2/8/8/8/4K3 w - -",
            Move::new_en_passant(F5, E6, WP, BP),
            false,
        );
        test_case(
            "8/2k5/8/4pP2/8/8/8/4K3 w - e6",
            Move::new_en_passant(F5, E6, WP, BP),
            true,
        );
        test_case(
            "8/2k5/r7/1p3P2/8/3B4/8/4K3 w K -",
            Move::new(D3, F5, WB, BB),
            false,
        );
        test_case(
            "8/2k5/r7/1p3P2/8/3B4/8/4K3 w K -",
            Move::new(D3, A6, WB, BR),
            false,
        );
        test_case(
            "8/2k5/r7/1p3P2/8/3B4/8/4K3 w K -",
            Move::new(D3, B5, WB, BP),
            true,
        );
        test_case(
            "8/2k5/1B6/qp3P2/8/6b1/5R2/4K3 w K -",
            Move::new(B6, A5, WB, BQ),
            true,
        );
        test_case(
            "8/2k5/1B6/qp3P2/8/6b1/5R2/4K3 w K -",
            Move::new(F2, D2, WR, NoPiece),
            true,
        );
        test_case(
            "8/2k5/1B6/qp3P2/8/6b1/5R2/4K3 w K -",
            Move::new(E1, D1, WK, NoPiece),
            true,
        );
        test_case(
            "8/2k5/1B6/qp3P2/8/6b1/5R2/4K3 w K -",
            Move::new(E1, D2, WK, NoPiece),
            false,
        );
        test_case(
            "8/3k4/1B6/qp3P2/8/6b1/4R3/4K3 w K -",
            Move::new(B6, A5, WB, BQ),
            false,
        );
        test_case(
            "8/2k5/1B6/qp3P2/8/6b1/4R3/4K3 w K -",
            Move::new(E2, D2, WR, NoPiece),
            false,
        );
        test_case(
            "8/2k5/1B6/qp3P2/8/6b1/4R3/4K3 w K -",
        ...
        test_case(
            ...
            Move::new(E2, E4, WP, NoPiece),
            true,
        );
        test_case(
            "4k3/8/8/8/8/4p3/4P3/4K3 w - -",
            Move::new(E2, E4, WP, NoPiece),
            false,
        );
        test_case(
            "r1bqk2r/pppp1ppp/2nbpn2/3N4/4P3/3B1N2/PPPP1PPP/R1BQK2R b KQkq -",
            Move::new(E6, D5, BP, WN),
            true,
        );
        test_case(
            "r2q1rk1/pb1p2pp/1pB1p3/2b2p2/2P5/4PN2/PPQ2PPP/R1B2RK1 b - - 0 4",
            Move::new(G1, H1, WK, NoPiece),
            false,
        );
    });
    ...
    chess_test!(test_legal_generation, {
        ...
        test_case(
            "rb2k1Nr/7p/5P1b/1qp5/8/2P5/P2P3P/RN2K1nR w KQkq - 0 1",
            &mut vec![
                "g8h6", "h1g1", "b1a3", "g8e7", "e1d1", "e1f2", "a2a3", "d2d3", "h2h3", "c3c4",
                "f6f7", "a2a4", "d2d4", "h2h4",
            ],
        );
        test_case(
            "rb2k1Nr/7p/5P1b/1qp5/8/2P5/P2P3P/RN2K1nR b KQkq - 0 1",
            &mut vec![
                "h6d2", "b8h2", "a8a2", "h8g8", "b5b1", "g1e2", "g1f3", "g1h3", "h6e3", "h6f4",
                "h6g5", "h6g7", "h6f8", "b8g3", "b8f4", "b8e5", "b8d6", "b8a7", "b8c7", "a8a3",
                "a8a4", "a8a5", "a8a6", "a8a7", "b5f1", "b5b2", "b5e2", "b5b3", "b5d3", "b5a4",
                "b5b4", "b5c4", "b5a5", "b5a6", "b5b6", "b5c6", "b5b7", "b5d7", "e8d7", "e8f7",
                "e8d8", "e8f8", "c5c4",
            ],
        );
        test_case(
            ...
            &mut vec![
                "e1g1", "e1c1", "a1b1", "a1c1", "a1d1", "a1a2", "a1a3", "a1a4", "a1a5", "a1a6",
                "a1a7", "a1a8", "h1f1", "h1g1", "h1h2", "h1h3", "h1h4", "h1h5", "h1h6", "h1h7",
                "h1h8", "e1d1", "e1f1", "e1d2", "e1e2", "e1f2",
            ],
        );
        test_case(
            "4k3/8/8/8/8/8/8/R3K2R w KQ - 0 1",
        ...
        test_case(
            "4k3/8/8/8/8/8/8/R3K2R w Kq - 0 1",
            &mut vec![
                "e1g1", "a1b1", "a1c1", "a1d1", "a1a2", "a1a3", "a1a4", "a1a5", "a1a6", "a1a7",
                "a1a8", "h1f1", "h1g1", "h1h2", "h1h3", "h1h4", "h1h5", "h1h6", "h1h7", "h1h8",
        ...
        test_case(
            ...
            &mut vec![
                "e8g8", "e8c8", "a8a1", "h8h1", "a8a2", "a8a3", "a8a4", "a8a5", "a8a6", "a8a7",
                "a8b8", "a8c8", "a8d8", "h8h2", "h8h3", "h8h4", "h8h5", "h8h6", "h8h7", "h8f8",
                "h8g8", "e8d7", "e8e7", "e8f7", "e8d8", "e8f8",
            ],
        );
        test_case(
            "r3k2r/8/8/2q5/8/8/8/R3K2R w KQkq - 0 1",
            &mut vec![
                "a1a8", "h1h8", "a1b1", "a1c1", "a1d1", "a1a2", "a1a3", "a1a4", "a1a5", "a1a6",
                "a1a7", "h1f1", "h1g1", "h1h2", "h1h3", "h1h4", "h1h5", "h1h6", "h1h7", "e1d1",
                "e1f1", "e1d2", "e1e2",
            ],
        );
        test_case(
            "4knn1/P5P1/3p4/PpP5/6n1/5b2/4PPP1/4K3 w - b6 0 1",
            &mut vec![
                "e1d1", "e1f1", "e1d2", "e2e3", "g2g3", "a5a6", "c5c6", "e2e4", "a7a8n", "a7a8b",
                "a7a8r", "a7a8q", "g7f8n", "g7f8b", "g7f8r", "g7f8q", "g2f3", "e2f3", "c5d6",
                "a5b6", "c5b6",
            ],
        );
        test_case(
            "4k3/p1pp1pb1/1n2pnp1/3PN3/Pp2P3/2N2Q1p/1PPBBPPP/R3K2R b KQ a3 0 1",
            &mut vec![
                "b6a4", "b6d5", "f6e4", "f6d5", "b6c4", "b6a8", "b6c8", "f6g4", "f6h5", "f6h7",
                "f6g8", "g7h6", "g7f8", "g7h8", "e8e7", "e8d8", "e8f8", "b4b3", "g6g5", "a7a6",
                "c7c6", "d7d6", "a7a5", "c7c5", "h3g2", "e6d5", "b4c3", "b4a3",
            ],
        );
        test_case(
            "8/1k6/5N2/2q5/6n1/3P4/5K2/6Q1 w - - 0 1",
        ...
        test_case(
            "k4b2/6P1/8/1pP5/8/8/6P1/7K w - b6 0 1",
            &mut vec![
                "h1g1", "h1h2", "g2g3", "c5c6", "g2g4", "g7g8n", "g7g8b", "g7g8r", "g7g8q",
                "g7f8n", "g7f8b", "g7f8r", "g7f8q", "c5b6",
            ],
        );
        test_case(
            "k4b2/6P1/8/1pP5/8/8/6P1/7K b - - 0 1",
            &mut vec![
                "f8c5", "f8g7", "f8d6", "f8e7", "a8a7", "a8b7", "a8b8", "b5b4",
            ],
        );
    });
