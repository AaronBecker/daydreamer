...
void flip_position(position_t* flipped, const position_t* src)
{
    ...
    flipped->side_to_move = flip_color(src->side_to_move);
...
bool is_move_legal(position_t* pos, move_t move)
{
    ...
    const color_t other_side = flip_color(pos->side_to_move);
    ...
    if (is_move_castle_long(move)) {
        ...
        if (castle_ok) {
            for (square_t sq = MIN(my_king_home, my_c1);
                ...
                if (is_square_attacked((position_t*)pos, sq, flip_color(side))) {
                    castle_ok = false;
                    break;
                }
            }
...
bool is_plausible_move_legal(position_t* pos, move_t move)
{
    ...
    if (!options.chess960) {
        if (is_move_castle_short(move) && !(has_oo_rights(pos, side) &&
                ...
                !is_square_attacked((position_t*)pos,my_king_home+1,flip_color(side)))) {
            return false;
        }
        ...
        if (is_move_castle_short(move)) {
            ...
            for (square_t sq = MIN(my_king_home, my_g1);
                ...
                if (sq != my_g1 &&
                        is_square_attacked((position_t*)pos, sq, flip_color(side))) {
                    return false;
                }
            }
        }
...
bool is_pseudo_move_legal(position_t* pos, move_t move)
{
    ...
    if (options.chess960 && is_move_castle(move)) {
        ...
        if (is_move_castle_long(move)) {
            ...
            bool castle_ok = !is_square_attacked(pos, to, flip_color(side));
    ...
    if (piece_is_type(piece, KING)) return !is_square_attacked(pos, to, flip_color(side));
    ...
    if (is_move_enpassant(move)) {
        undo_info_t undo;
        do_move(pos, move, &undo);
        bool legal = !is_square_attacked(pos, pos->pieces[side][0], flip_color(side));
        undo_move(pos, move, &undo);
        return legal;
    }
    return true;
